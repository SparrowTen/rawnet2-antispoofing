...
class ASVDataset(Dataset):
    ...
    def __init__(
        ...
        self.protocols_fname = (
            "eval.trl" if is_eval else "train.trn" if is_train else "dev.trl"
        )
        ...
        self.files_dir = os.path.join(
            self.data_root_dir, "{}_{}".format(self.prefix, self.dset_name), "flac"
        )
        ...
        if sample_size:
            select_idx = np.random.choice(
                len(self.files_meta), size=(sample_size,), replace=True
            ).astype(np.int32)
